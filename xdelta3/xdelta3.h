...
static_assert(SIZEOF_UNSIGNED_LONG_LONG == sizeof(unsigned long long),
...
#endif
#define short_sprintf(sb, fmt, ...) snprintf_func((sb).buf, sizeof((sb).buf), fmt, __VA_ARGS__)
...
/* Underlying xprintf() */
int xsnprintf_func(char* str, size_t n, const char* fmt, ...) PRINTF_ATTRIBUTE(3, 4);
...
int xd3_encode_memory(const uint8_t* input, usize_t input_size, const uint8_t* source,
                      usize_t source_size, uint8_t* output_buffer, usize_t* output_size,
                      usize_t avail_output, int flags);

/* The reverse of xd3_encode_memory. */
int xd3_decode_memory(const uint8_t* input, usize_t input_size, const uint8_t* source,
                      usize_t source_size, uint8_t* output_buf, usize_t* output_size,
                      usize_t avail_output, int flags);
 ...
 */
int xd3_encode_stream(xd3_stream* stream, const uint8_t* input, usize_t input_size, uint8_t* output,
                      usize_t* output_size, usize_t avail_output);

/* The reverse of xd3_encode_stream. */
int xd3_decode_stream(xd3_stream* stream, const uint8_t* input, usize_t input_size, uint8_t* output,
                      usize_t* output_size, usize_t avail_size);
 ...
 * is returned again.
 */
static inline void xd3_avail_input(xd3_stream* stream, const uint8_t* idata, usize_t isize) {
 ...
 * XD3_OUTPUT return. */
static inline void xd3_consume_output(xd3_stream* stream) { stream->avail_out = 0; }
...
static inline xoff_t xd3_encoder_srcbase(xd3_stream* stream) { return stream->src->srcbase; }
static inline usize_t xd3_encoder_srclen(xd3_stream* stream) { return stream->src->srclen; }
...
static inline void xd3_blksize_div(const xoff_t offset, const xd3_source* source, xoff_t* blkno,
                                   usize_t* blkoff) {
...
static inline void xd3_blksize_add(xoff_t* blkno, usize_t* blkoff, const xd3_source* source,
                                   const usize_t add) {
    usize_t blkdiff;
